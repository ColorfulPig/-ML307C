...
void cm_gnss_nmea_all_callback_func(const char *nmea, uint32_t len)
{
	cm_demo_printf("%s",(char *)nmea);
}

void cm_gnss_nmea_callback_func(uint32_t type, const char *nmea, uint32_t len)
{
	cm_demo_printf("%s",(char *)nmea);
}

void rsp_callback(const char *data, uint32_t len)
{
	cm_demo_printf("rsp:%s",(char *)data);
}
...
void cm_test_gnss_enable(unsigned char **cmd,int len)
{
	...
	cm_demo_printf("enable_param:%d",enable_param);
    if(enable_param == 1)
    {
	...
	else if(2 == enable_param)
	{
		cm_gnss_nmea_all_callback_regist(cm_gnss_nmea_all_callback_func);
	}
	else if(3 == enable_param)
	{
        cm_gnss_rawdata_callback_regist(rsp_callback); 
	}
    else
    {
        cm_gnss_close();
...
void cm_test_gnss_sendrawdata(unsigned char **cmd,int len)
{
    ...
    cm_gnss_send_raw_data(senddata, senddata_len);
}
